...
# 13. Terms to Avoid or Use Carefully

## Task

Avoid using `Task` alone inside the domain model.
...
A generic task sounds like project-management software. Snaptuki coordinates care work, so the domain term should be CareTask.
...
In Authentication or User Account Management, `User` may mean login account.

In Care Task Coordination, prefer:
...
A login user and a care role are not always the same concept.

---

## Patient

Avoid unless the nursing home explicitly uses this term.
...
The project is focused on nursing homes and elderly care, where “resident” is often more appropriate than “patient”.
...
It is shorter, cleaner, and easier to use consistently in code and documentation.

---

## Reactivated

Do not use as a status.

Preferred usage:

```text
Command: ReactivateCareTask
Event: CareTaskReactivated
Resulting status: Planned or Assigned
```
